...
2. **Wait a few seconds** — UniForm's Iceberg metadata generation is
   asynchronous; this is the only remaining delay. No manual
   `REFRESH EXTERNAL TABLE` step is needed anymore — the `databricks_uc`
   catalog has `iceberg_meta_cache_ttl_sec = 0` set, so a plain `SELECT`
   (exactly what the dashboard backend issues) always fetches fresh
   metadata. A dashboard user querying after someone else's Databricks edit
   sees it automatically, with nobody running any StarRocks-side command in
   between.
