...
This also auto-loads `.env` (confirmed: `DATABRICKS_CATALOG` and friends are
populated with no manual `source .env` needed) — for a host-side StarRocks
connection you still need to point `STARROCKS_URL` at `localhost` rather
than the in-Docker-network default:
